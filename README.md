...
## Деплой
Библиотеки, не сервис: Helm/terraform не используются; публикация — в приватные реестры AI37 через GitHub Actions вручную (`workflow_dispatch`, опция `dry_run` — сборка и проверки без заливки). Текущие версии: `@ai37/agent-sdk` — `0.1.0-alpha.18` (TS), `ai37-agent-sdk` — `0.1.0a10` (Python), `@ai37/agent-host` — `0.1.0-alpha.38` (публикуется независимо от SDK).
