/**
 * Versioned AI37 extension URI used in A2A Agent Card
 * `capabilities.extensions`. The URI is also the contract identifier.
 */
...
/**
 * Compact routing projection. It deliberately contains no prose Agent Card
 * description, examples, endpoints, authentication or billing metadata.
 */
...
const intentSet = new Set<string>(AI37_ROUTING_INTENTS)
  ...
  for (const item of value) {
    ...
    if (!normalized || normalized.length > limit.length) {
      throw new RangeError(
        `routing.${field} items must be 1..${limit.length} characters`,
      )
    }
...
function normalizeIntents(value: unknown): AgentRoutingIntent[] {
  ...
  if (value.length > LIMITS.intents.items) {
    throw new RangeError(
      `routing.intents must contain at most ${LIMITS.intents.items} items`,
    )
  }
  const result: AgentRoutingIntent[] = []
  const seen = new Set<string>()
  for (const item of value) {
    if (typeof item !== 'string' || !intentSet.has(item)) {
...
export function normalizeAgentRoutingProfile(
  value: unknown,
): AgentRoutingProfile {
...
/**
 * Reads only the canonical versioned extension. Unknown/malformed extensions
 * are ignored so that registry discovery remains forward-compatible.
 */
