...
- Парити с TS `0.1.0-alpha.27`: подсчёт длины витрины в кодовых точках зафиксирован тестом
  (в Python `len()` и так их считает — правка поведения нужна была TS-пакету). Общий
  проверочный пример: заголовок из 60 кодовых точек с эмодзи не обрезается ни в одном SDK.
