...
export interface ContextFile {
  ...
  isLarge?: boolean
  /**
   * MIME-тип файла из store. Даёт агенту выбрать НУЖНОЕ вложение, не скачав ни одного — когда их
   * несколько, имя различает их плохо. Это ЗАЯВЛЕНИЕ о файле (на загрузке оно берётся из
   * markitdown, иначе от браузера), поэтому маршрутизировать по нему можно, а отдавать парсеру на
   * его основании — нет. Опционально: продюсер постарше его не пришлёт.
   */
  mime?: string
  /**
   * Сохранены ли сырые байты оригинала (chat-тир под cap). true — агент может забрать их через
   * `readRaw`; false/undefined — только markdown. Нужно для детерминированного парсинга агентом
   * (openpyxl, картинки), НЕ для LLM.
   */
  hasRaw?: boolean
}
...
export interface Ai37Metadata {
  ...
  instructions?: string
  /**
   * Клиент перепрогоняет ПОСЛЕДНИЙ ход треда, а не задаёт новый вопрос («Заново» под ответом).
   * Потребитель — оркестратор: перед обработкой он откатывает хвост последнего хода, чтобы вопрос
   * не задвоился в истории. Без флага перепрогон неотличим от законного повторного сообщения
   * («ещё», «продолжай»), а угадывать по совпадению текста нельзя — ценой ошибки будет удаление
   * настоящего ответа.
   *
   * Свойство хода UI↔оркестратор: вниз сабагентам НЕ форвардится (их задача перепрогоном не
   * является). Носитель — только AG-UI (`forwardedProps.ai37`), как у `acceptedOutputModes`.
   */
  rerun_last_turn?: boolean
}
