...
class ContextFile:
    ...
    #: MIME-тип из store: по нему агент выбирает нужное вложение, не скачав ни одного. Это
    #: ЗАЯВЛЕНИЕ о файле (markitdown, иначе браузер), маршрутизировать можно, доверять парсеру нет.
    mime: str | None = None
    #: Сохранены ли сырые байты оригинала (chat-тир под cap) — для детерминированного парсинга
    #: агентом, не для LLM. ``None`` — продюсер поля не прислал.
